...
BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # path to classical_models folder
OBJ_DIR = os.path.join(BASE_DIR, "objects")

encoder = pickle.load(open(os.path.join(OBJ_DIR, 'encoder.pkl'), 'rb'))
scaler = pickle.load(open(os.path.join(OBJ_DIR, 'scaler.pkl'), 'rb'))
feature_cols = pickle.load(open(os.path.join(OBJ_DIR, 'feature_cols.pkl'), 'rb'))
city_list = pickle.load(open(os.path.join(OBJ_DIR, 'city_list.pkl'), 'rb'))
model = pickle.load(open(os.path.join(OBJ_DIR, 'model_no_3.pkl'), 'rb'))
...
# Correct images folder path
IMAGE_FOLDER = os.path.join(BASE_DIR, "images")

# Ensure images folder exists
if not os.path.exists(IMAGE_FOLDER):
    st.error(f"Images folder not found: {IMAGE_FOLDER}")
...
if image_files:
    ...
    st.write("Using image:", image_path)   # debug path

    # Image title formatting
    image_title = (
        image_files[image_index][2:]
        .split('.')[0]
        .replace('_', ' ')
        .title()
    )
